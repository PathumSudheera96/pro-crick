...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   CREATE TYPE "public"."enum_redirects_redirect_type" AS ENUM('301', '302');
  CREATE TABLE "redirects" (
  	"id" serial PRIMARY KEY NOT NULL,
  	"from_path" varchar NOT NULL,
  	"to_path" varchar NOT NULL,
  	"redirect_type" "enum_redirects_redirect_type" DEFAULT '301' NOT NULL,
...
export async function down({ db }: MigrateDownArgs): Promise<void> {
  ...
  DROP TYPE "public"."enum_redirects_redirect_type";`)
}
